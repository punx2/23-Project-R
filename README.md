...
### 1. 두변수의 상관 관계
 - presure 데이터셋을 통하여 온도와 기압의 관련성 분석
``` R
# (1) 자료 확인
head(pressure)
...
```
- 상관 계수 : 상관 관계를 수치로 나타낸 -1에서 1사이의 값
  - 상관 계수가 0인 경우 : x, y 사이에 상관성 찾기 어려움
  - 상관 계수가 - 인 경우 : x,y가 반비례, 음의 상관 관계가 있음
  - 상관 계수가 + 인 경우 : x,y가 비례, 양의 상관 관계가 있음
...
### 2. 다중 변수 사이의 상관 관계
 - 변수가 3개 이상인 데이터는 변수를 2개씩 짝지어서 상관관계 분석 가능
...
- 데이터 전처리
   ...
   - 데이터 전처리는 전체 분서에서 오랜시간을 차지
- 결측값 처리
   - 데이터 수집, 저장 과정에서 값을 못하는 경우 발생 (NA로 표현)
   - 결측값이 섞여있는 데이터 셋은 분석할때 문제를 일으킴
   - 2가지 방법으로 처리
      - 결측값을 제거 또는 제외한 후 분석
...
- NA를 제외하고 계산하는 방법
``` R
z <- c(1,2,3,NA,5,NA,8)      # 결측값이 포함된 벡터 z 생성
sum(z)                       # 정상 계산이 되지 않음
is.na(z)                     # NA 여부 확인
...
- NA 를 다른 값으로 대체하는 방법과 NA 제거하는 방법
...
# 1부터 컬럼 수만큼 반복
for (i in 1:ncol(x)) { 
  # x 변수의 i열에 해당하는 값이 na인지 true / false로 저장
  this.na <- is.na(x[,i])
  # x의 i컬럼의 이름, tab 키 입력, na수, 줄바꿈 출력ㄴ 
...
- complete cases 를 이용하여 NA 가 포함된 행들을 나타내거나 제거하는 방법
```R
head(x)
